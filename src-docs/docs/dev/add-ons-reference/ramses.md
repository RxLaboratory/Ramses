...
| Attribute | Type | Default | Description |
| --- | --- | --- | --- |
| **importScripts** | *list* | `[]` | A list of scripts/functions to be triggered when `Ramses.instance().importItem()` is called.<br />If you're using one of the provided Add-ons, you can add your own callbacks to this list so they're run when the user asks to import an item. |
...
| **replaceScripts** | *list* | `[]` | A list of scripts/functions to be triggered when `Ramses.instance().replaceItem()` is called.<br />If you're using one of the provided Add-ons, you can add your own callbacks to this list so they're run when the user asks to import an item. |
...
| **publish** | *[RamItem](ram_item.md) or [RamAsset](ram_asset.md) or [RamShot](ram_shot)*: **item**,<br />*[RamStep](ram_step.md)*: **step**,<br />*string*: **publishFolderPath**,<br />*[RamFileInfo](ram_file_info.md)*: **fileInfo** | Runs the scripts in `Ramses.instance().publishScripts`.<br />Read the section entitled *Callbacks* below for more details. |
| **replaceItem** | *[RamItem](ram_item.md) or [RamAsset](ram_asset.md) or [RamShot](ram_shot)*: **item**,<br />*string*: **filePath**,<br />*string*: **stepShortName**=`""` | Runs the scripts in `Ramses.instance().replaceScripts`.<br />Read the section entitled *Callbacks* below for more details. |
...
| **publish** | • *[RamItem](ram_item.md) or [RamAsset](ram_asset.md) or [RamShot](ram_shot)*: **item**,<br />• *[RamStep](ram_step.md)*: **step**,<br />• *[RamFileInfo](ram_file_info.md)*: **publishFileInfo** | - `item` is the item from which the user wants to publish a file.<br />- `step` is the step from which the user is publishing. You can check `step.outputPipes()` to know which file types to publish.<br />- `fileInfo` contains all the info which can be gathered from the publishing file, including version and state. `fileInfo.filePath()` returns the path for publishing the file (including a proposed fileName). |
| **replaceItem** | • *[RamItem](ram_item.md) or [RamAsset](ram_asset.md) or [RamShot](ram_shot)*: **item**,<br />• *list* of *string*: **filePaths**,<br />• *[RamStep](ram_step.md)*: **step** | - `item` is the item from which the user wants to import a file.<br />- `filePath` is the absolute path to the file the user wants to import.<br />- `step` is the step from which the user wants to import. You can check `step.outputPipes()` to know which file types have been published, and `inputPipes()` from the step you're importing to to know which files to accept.<br />With this info, the method should replace selected objects with the imported items. |
