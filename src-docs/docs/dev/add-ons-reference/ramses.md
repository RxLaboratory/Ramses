...
| **publish** | • *[RamItem](ram_item.md) or [RamAsset](ram_asset.md) or [RamShot](ram_shot)*: **item**,<br />• *[RamStep](ram_step.md)*: **step**,<br />• *string*: **publishFolderPath**,<br />• *[RamFileInfo](ram_file_info.md)*: **fileInfo** | - `item` is the item from which the user wants to publish a file.<br />- `step` is the step from which the user is publishing. You can check `step.outputPipes()` to know which file types to publish.<br />- `publishFolderPath` is the absolute path to the folder where the user wants to publish.<br />- `fileInfo` contains all the info which can be gathered from the publishing file, including version and state. |
